		...
		<table class="table table-border table-bordered table-bg">
			...
			<c:forEach items="${usersList}" var="users">
				<tr class="text-c">
					...
					<td>${users.status}</td>
					<td>${users.regdate}</td>
					<td><a href="updateUserStatus?id=${users.usersid}">${users.status == "解锁"?"锁定":"解除锁定"}</a>&nbsp;&nbsp;<a
						href="deleteUsers?id=${users.usersid}" onclick="{if(confirm('确定要删除吗?')){return true;}return false;}"><i
